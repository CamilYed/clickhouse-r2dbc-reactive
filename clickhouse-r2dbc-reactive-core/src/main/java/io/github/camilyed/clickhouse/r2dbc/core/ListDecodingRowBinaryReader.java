 ...
 * .internal} package that this project otherwise deliberately avoids depending on (see
 * ROADMAP.md's Phase 0 reuse-boundary decision). {@code BinaryStreamReader} itself already
 * special-cases a {@code List.class} type hint to return a plain {@code List} instead; this class
 * is the one narrow, documented place that supplies it, by overriding the {@code protected} {@code
...
final class ListDecodingRowBinaryReader extends RowBinaryWithNamesAndTypesFormatReader {
    ...
    private static Class<?> listHintFor(final ClickHouseColumn column) {
        ...
        final boolean isListLike = dataType == ClickHouseDataType.Array || dataType == ClickHouseDataType.Nested;
