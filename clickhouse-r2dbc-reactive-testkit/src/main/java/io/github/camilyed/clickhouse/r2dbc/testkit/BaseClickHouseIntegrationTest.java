 ...
 *
 * <p>Uses Testcontainers' singleton-container pattern: the {@link ClickHouseContainer} is a
 * single {@code static} field declared here, so every test class that extends this one shares the
 * same running container instead of each starting its own — one container for the whole test JVM.
 ...
 *
 * <p>{@link #dropAllTables()} runs before every test and drops every table in the default
 * database, so each test starts from a clean, known state — the same isolation rule CLAUDE.md
 * requires of in-memory fakes, applied here to a real database. It talks to ClickHouse over a
 * plain, synchronous {@link HttpClient}, deliberately not through this project's own reactive
 * transport, so cleanup never depends on — or silently masks a bug in — the very driver code these
...
public abstract class BaseClickHouseIntegrationTest {
    ...
    @Container
    private static final ClickHouseContainer CLICK_HOUSE = new ClickHouseContainer("clickhouse/clickhouse-server:latest");
    ...
    protected void dropAllTables() {
        queryColumn("SHOW TABLES").forEach(table -> executeAdminSql("DROP TABLE IF EXISTS `" + table + "`"));
    }
    ...
    private static String sendAdminRequest(final String sql) {
        ...
        final String basicAuth = Base64.getEncoder().encodeToString(credentials.getBytes(StandardCharsets.UTF_8));
        final HttpRequest request = HttpRequest.newBuilder()
                .uri(URI.create(clickHouseHttpUrl() + "/?query=" + URLEncoder.encode(sql, StandardCharsets.UTF_8)))
                .header("Authorization", "Basic " + basicAuth)
                .timeout(Duration.ofSeconds(10))
                .POST(HttpRequest.BodyPublishers.noBody())
                .build();
        try {
            final HttpResponse<String> response = ADMIN_HTTP_CLIENT.send(request, HttpResponse.BodyHandlers.ofString());
            if (response.statusCode() >= 400) {
                throw new IllegalStateException("Admin SQL failed (" + response.statusCode() + "): " + response.body());
            }
