 ...
 *
 * <p>{@link #execute()} is not implemented yet — that, along with {@link Result} and its supporting
 * {@code Row}/{@code RowMetadata} types, is the next slice of Phase 3 work. Parameter binding
 * ({@link #bind}/{@link #bindNull}/{@link #add}) is separately scoped future work (needs a {@code
 * param_<name>} bind-parameter design, not yet decided) and is left unimplemented here for the same
 * reason: nothing in this class silently pretends to support what it doesn't.
 */
final class ClickHouseStatement implements Statement {
  ...
  public Publisher<? extends Result> execute() {
    return Mono.error(
        new UnsupportedOperationException("Statement.execute() is not implemented yet"));
  }
}
