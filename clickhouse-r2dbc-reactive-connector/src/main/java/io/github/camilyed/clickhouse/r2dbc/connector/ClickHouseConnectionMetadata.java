 ...
 *
 * <p>{@link #getDatabaseVersion()} is a fixed {@code "unknown"} for now — reporting the real server
 * version needs a query round trip ({@code SELECT version()}) that nothing wires up yet. A
 * deliberate, documented gap rather than a guess, not a design decision to revisit later.
 */
