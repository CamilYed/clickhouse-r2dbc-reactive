...
class ClickHouseConnectionFactoryAgainstRealClickHouseTest extends BaseClickHouseIntegrationTest {
  ...
  /**
   * Proves a {@code user} option with no accompanying {@code password} authenticates with a truly
   * empty password server-side, not the literal string {@code "null"} a naive {@code
   * String.valueOf(password)} on a {@code null} password would have sent.
   */
  @Test
  void shouldAuthenticateWithEmptyPasswordWhenUserIsPresentAndPasswordIsAbsent() {
    // given
    createUserWithPassword("empty_password_user", "");
    final URI httpUrl = URI.create(clickHouseHttpUrl());
    final ConnectionFactoryOptions options =
        ConnectionFactoryOptions.builder()
            .option(ConnectionFactoryOptions.DRIVER, "clickhouse")
            .option(ConnectionFactoryOptions.HOST, httpUrl.getHost())
            .option(ConnectionFactoryOptions.PORT, httpUrl.getPort())
            .option(ConnectionFactoryOptions.USER, "empty_password_user")
            .build();
    final ClickHouseConnectionFactory factory = ClickHouseConnectionFactory.from(options);
  ...
  void shouldNotAuthenticateAgainstALiteralNullPasswordWhenPasswordOptionIsAbsent() {
    // given
    createUserWithPassword("literal_null_password_user", "null");
    final URI httpUrl = URI.create(clickHouseHttpUrl());
    final ConnectionFactoryOptions options =
        ConnectionFactoryOptions.builder()
            .option(ConnectionFactoryOptions.DRIVER, "clickhouse")
            .option(ConnectionFactoryOptions.HOST, httpUrl.getHost())
            .option(ConnectionFactoryOptions.PORT, httpUrl.getPort())
            .option(ConnectionFactoryOptions.USER, "literal_null_password_user")
            .build();
    final ClickHouseConnectionFactory factory = ClickHouseConnectionFactory.from(options);
