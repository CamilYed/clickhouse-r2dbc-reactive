 ...
 * {@link ClientV2PointQueryClient#ClientV2PointQueryClient(double)} both append {@code
 * sleep(}{@link #sleepSeconds}{@code )} to the query — a real, deterministic, server-side hold —
 * long enough that a {@code concurrency} value above the pool size actually forces queueing on the
 * smaller side, which is the whole point of comparing two <em>different</em> pool sizes rather than
 * a matched one.
 *
