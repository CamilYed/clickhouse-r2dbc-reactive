 ...
 *
 * <p>Root cause, confirmed against {@code RowBinaryDecoder.decode}'s source, not just inferred
 * from the failure: {@code Mono.fromCallable(() -> newReader(source, compression))
 ...
 * incidentally this driver's real admission-control gate on how many queries can start touching
 * the connection pool at once. Coupling it to {@code poolSize} (the default) keeps that number
 * under Reactor Netty's own pending-acquire-queue limit ({@code 2 × maxConnections} by default);
 * widening it alone removes that incidental protection without replacing it with anything, so at
 * concurrency above the pool size the pending-acquire-queue limit becomes a hard failure instead
 * of the added latency PR5 originally set out to shrink.
 *
 ...
 * unavoidable bottleneck either way) — should recover the {@code concurrency=8} tail-latency win
 * at {@code concurrency=32}/{@code 128} too, without the outright failures widening the decoder
 * alone produced. {@link #WIDENED_PENDING_ACQUIRE_MAX_COUNT} is sized with the same "deliberately
