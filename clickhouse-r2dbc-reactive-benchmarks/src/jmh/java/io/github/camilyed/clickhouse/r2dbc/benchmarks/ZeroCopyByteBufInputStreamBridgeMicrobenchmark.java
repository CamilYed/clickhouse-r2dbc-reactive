 ...
 * call performs cost anything measurable, in isolation from HTTP/Docker/network noise? That
 * class's trusted 3-fork run showed a ≤1.3% difference that flipped sign between scenarios (`point`
 * slower with the copy removed, `SELECT 1` faster) — too small and inconsistent to separate "the
 * copy genuinely doesn't matter" from "the effect is real but buried under this machine's
 ...
 * wait (see that class's own decisive-negative result: ~40-50ns/chunk, ~200x smaller than the
 * production per-chunk figure it was checking against — the same style of verdict this class exists
 * to reach for the copy question specifically).
 *
 ...
 * the chunk-size tiers {@link FluxInputStreamBridgeMicrobenchmark} already covers, for context. Each
 * response here is always a single chunk (no coalescing involved) — matching the real shape of a
 * {@code SELECT 1}/point response, which this class exists to explain.
 *
 * <p>Before trusting any run, verify no leak with the same {@code
 * -Dio.netty.leakDetection.level=paranoid} / {@code LeakRecordingResourceLeakDetector} wiring {@code
 * LatencyPathVariantBBenchmark} uses (see docs/performance/latency-path-isolation.md).
 */
...
public class ZeroCopyByteBufInputStreamBridgeMicrobenchmark {
  ...
  /** Mirrors {@code RowBinaryDecoder.RESPONSE_CHUNK_DEMAND} — see {@link LatencyPathVariantBBenchmark}. */
  ...
  /** {@code asByteArray()}-equivalent copy, then {@link FluxInputStreamBridge} — the production path. */
  @Benchmark
