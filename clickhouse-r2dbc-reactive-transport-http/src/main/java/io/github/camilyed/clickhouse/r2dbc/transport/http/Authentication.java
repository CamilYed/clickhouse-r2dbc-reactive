 ...
 * it needs a real client-certificate {@code SSLContext} wired into the underlying {@code HttpClient},
 * not just a header, so it's tracked as separate, larger work rather than half-built).
 *
...
public sealed interface Authentication {
     ...
     * X-ClickHouse-Key}. The alternative client-v2 falls back to when HTTP Basic auth is turned
     * off; useful against a server/proxy that strips or mishandles the {@code Authorization} header.
     */
    ...
    /** ClickHouse's own header-pair authentication: {@code X-ClickHouse-User} / {@code X-ClickHouse-Key}. */
