 ...
 *
 * <p>client-v2's own {@code ClickHouseHttpProto} source carries a suspicious contradiction:
 * {@code HEADER_QUERY_ID}'s Javadoc says "Response only header ... Cannot be used in request", yet
 * client-v2's own {@code addHeaders} code sets it as a <b>request</b> header anyway (and separately
 * also sends a {@code query_id} URL query parameter). We already send {@code
 * X-ClickHouse-Query-Id} as a request header in {@link ClickHouseHttpTransport#query}, proven only
 * against our own controlled fake server, which trivially echoes back whatever header it receives —
 * that proves nothing about whether a <em>real</em> ClickHouse server actually honors it. This test
 * settles that empirically: send an explicit {@code query_id} as a request header only (no query
...
class QueryIdHeaderAgainstRealClickHouseTest extends BaseClickHouseIntegrationTest {
    ...
    void shouldEchoBackTheQueryIdWeSentAsARequestHeader() {
        ...
        final String echoedQueryId = httpClient
                ...
                .response((response, content) -> content.aggregate()
                        .asString()
