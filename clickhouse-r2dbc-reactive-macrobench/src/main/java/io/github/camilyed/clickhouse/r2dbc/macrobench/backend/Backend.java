 ...
 * #CLIENT_V2} alone mean the other backend's beans are never created at all - required for
 * trusted CPU/RSS/thread measurements, since an idle backend's own threads still contaminate
 * process-level resource numbers even with zero traffic reaching it. See ROADMAP.md's Phase 12.
 */
public enum Backend {
  R2DBC,
  CLIENT_V2,
  ...
  /**
   * Parses {@code benchmark.backend}/REST-path-style values ({@code "r2dbc"}, {@code
   * "client-v2"}, {@code "dual"}), case-insensitive - the one place the hyphenated {@code
   * "client-v2"} spelling is translated to the {@link #CLIENT_V2} enum constant, shared by {@code
