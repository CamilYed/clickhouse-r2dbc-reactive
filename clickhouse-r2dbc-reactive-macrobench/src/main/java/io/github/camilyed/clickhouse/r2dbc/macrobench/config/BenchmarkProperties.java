 ...
 * large the seeded dataset is. Defaults here are a local-smoke-test size, not ROADMAP.md's Phase 12
 * "trusted" sizing (~5M-row fact table) - override via {@code benchmark.point-rows}/{@code
 * benchmark.analytics-rows} (or the {@code MACROBENCH_POINT_ROWS}/{@code MACROBENCH_ANALYTICS_ROWS}
 * environment variables application.yml maps them to) for a real run.
 */
