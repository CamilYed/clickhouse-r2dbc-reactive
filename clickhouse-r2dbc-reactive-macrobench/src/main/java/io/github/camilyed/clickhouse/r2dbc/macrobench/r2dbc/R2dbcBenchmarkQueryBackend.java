...
/**
 * {@link BenchmarkQueryBackend} through this driver, via Spring's {@link DatabaseClient} - the same
 * "fully pre-formed SQL, literal values, no {@code .bind(...)}" style {@code
 * examples/spring-boot-webflux-demo}'s {@code DatabaseClientOrderEventRepository} already uses (see
 * that class's Javadoc for why {@code .bind(...)} doesn't work against this driver's {@code
 * {name:Type}} parameter syntax through {@link DatabaseClient} yet). Every value embedded by this
 * class is a driver-generated {@code long} (never user input), so no literal-escaping is needed the
 * way the demo's string/array values require.
 */
final class R2dbcBenchmarkQueryBackend implements BenchmarkQueryBackend {
  ...
  public Mono<PointRow> point(final long id) {
    return databaseClient
        .sql("SELECT id, label, amount FROM " + BenchmarkDataset.POINT_TABLE + " WHERE id = " + id)
  ...
  private static PointRow toPointRow(final Readable row) {
    return new PointRow(
  ...
  private static CategoryTotal toCategoryTotal(final Readable row) {
